 ...
 * класса \Bendersay\Entityadmin\EntityManager.
 *
 * @see EntityManager
...
{
    public const LIST_HELPER = 1;
    public const EDIT_HELPER = 2;
    ...
    /**
     * @var string|null Заголовок поля. Используется для человеко-понятных названий в административной части сайта.
     */
    protected ?string $title = null;

    /**
     * @var bool Обязательность поля
     */
    protected bool $required = false;

    /**
     * @var array Значения списка
     */
     ...
     * @api
     */
    abstract protected function getEditHtml(): string;
     ...
     *
     * - false - отобразить теги в виде текста. Результат обработки: <b>text</b> = &lt;b&gt;text&lt;/b&gt;
     *
     * @return string
     */
    public static function prepareToOutput(?string $string, bool $hideTags = true): string
    {
        if ($hideTags) {
    ...
    public function showBasicEditField($isPKField)
    {
        ...
        print '<tr>';
        $title = $this->getTitle() ?? $this->getCode();
    ...
    public function changeGetListOptions(&$filter, &$select, &$sort, $raw)
    {
        if ($this->isFilterBetween()) {
            ...
            if ($filterPrefix = $this->getSettings(
                'FILTER'
            ) and $filterPrefix !== true and isset($filter[$this->getCode()])) {
    ...
    /**
     * Устанавливаем значения списка
     *
     * @param array $enumList
     */
    public function setEnumList(array $enumList): void
    {
    ...
    {
        return Loc::getMessage('zebrains_AH_MULTI_NOT_SUPPORT');
    }
    ...
    protected function addError($messageId, $replace = [])
    {
        $this->validationErrors[$this->getCode()] = Loc::getMessage(
            $messageId,
            array_merge(['#FIELD#' => $this->getSettings('TITLE')], $replace)
        );
    }
    ...
    protected function getFilterInputName()
    {
        if ($this->isFilterBetween()) {
            ...
            return [$inputNameFrom, $inputNameTo];
        } else {
    ...
    {
        if (is_a($this->helper, 'Bendersay\Entityadmin\Helper\AdminListHelper')) {
            return self::LIST_HELPER;
        } else {
            if (is_a($this->helper, 'Bendersay\Entityadmin\Helper\AdminEditHelper')) {
